...
class ActualController extends Controller
{
    ...
    {   
        ...
        if ($buscar==''){
            ...
            ->select('actual.id', 'actual.unidad', 'actual.codigo', 'codcont.nombre',
                     ...
                     'actual.codaux')
            ->distinct('resp.id')
            ->where('actual.unidad','=',$request->unidad)->paginate(10);
            ...
            ->select('actual.id', 'actual.unidad', 'actual.codigo', 'codcont.nombre',
                     ...
                     'actual.cod_rube', 'actual.codigosec', 'actual.observ', 'actual.codcont', 
                     'actual.codaux')
            ->distinct('resp.id')->where('actual.unidad','=',$request->unidad)
    ...
    public function repAsignaciones(Request $request){
        ...
        $responsable = Responsables::join('oficina','resp.codofic','=','oficina.codofic')
                                    ...
                                    ->select('unidadadmin.descrip as unidad','resp.nomresp','oficina.nomofic','resp.cargo','oficina.codofic','resp.ci','cla_depts.sigla')
